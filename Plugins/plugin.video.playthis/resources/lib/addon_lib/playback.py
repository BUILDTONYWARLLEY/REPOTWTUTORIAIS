...
def resolve(url, title=''):
    ...
    log_utils.log('Attempting to resolve: |{0!s}|''---VIDA---', log_utils.LOGDEBUG)
...
def play_this(item, title='', thumbnail='', player=True, history=None):
    ...
    if item.startswith('http'):
        with kodi.ProgressDialog('%s...' % kodi.i18n('resolving'), '%s:' % kodi.i18n('attempting_determine_type'), '---VIDA---') as progress_dialog:
            while not progress_dialog.is_canceled():
                url_override = __check_for_new_url(item)
                if item != url_override:
                    log_utils.log('Source |{0}| has been replaced by |{1}|''---VIDA---', log_utils.LOGDEBUG)
                    progress_dialog.update(5, '%s: %s' % (kodi.i18n('source'), '---VIDA---'), '%s: %s' % (kodi.i18n('replaced_with'), '---VIDA---'), ' ')
                    item = url_override
                ...
                if url_override:
                    log_utils.log('Source |{0}| has been replaced by |{1}|''---VIDA---', log_utils.LOGDEBUG)
                    progress_dialog.update(10, '%s: %s' % (kodi.i18n('source'), '---VIDA---'), '%s: %s' % (kodi.i18n('replaced_with'), '---VIDA---'), ' ')
                    item = url_override
