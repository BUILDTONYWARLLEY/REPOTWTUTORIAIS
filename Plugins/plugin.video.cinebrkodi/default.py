...
def SKindex():
    ...
    addDir('[B]Contrate Sua Lista Ou Solicite seu Teste por 3 hrs[/B]','url',102,icon,FANART,'','','','')
    addDir('[B]Minha Lista Iptv Clique aqui[/B]','url',111,icon,FANART,'','','','')
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if regexs: 
            ...
            if addon.getSetting('dlaudioonly') == 'true':
                ...
                url = 'plugin://plugin.video.f4mTester/?url=' +url+ '&amp;streamtype=TSDOWNLOADER&amp;name=cinebrkodi'
                mode = '12'
...
if mode==None:
	...
	import web
	url ="http://ns1.xxhost.top/~cinebrko/contato/cinebr.html"
	web.web(url)
	xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==102:
	import web
	url ="http://ns1.xxhost.top/~cinebrko/contato/contrate.html"
	web.web(url)
	xbmcplugin.endOfDirectory(int(sys.argv[1]))
