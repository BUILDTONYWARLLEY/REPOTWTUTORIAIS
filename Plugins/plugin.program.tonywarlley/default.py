		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count18 > 0:
					...
					addFile('[B]%s Builds TONYWARLLEY Kodi Leia Alpha 18(%s)[/B]' % (state, count18), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW18 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count17 > 0:
					...
					addFile('[B]%s Builds TONYWARLLEY Kodi Krypton 17(%s)[/B]' % (state, count17), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW17 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count16 > 0:
					...
					addFile('[B]%s Builds TONYWARLLEY Kodi Jarvis 16(%s)[/B]' % (state, count16), 'togglesetting',  'show16', themeit=THEME3)
					if SHOW16 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def viewBuild(name):
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addDir ('Salvar Dados',       'savedata', icon=ICONSAVE,     themeit=THEME3)
		...
		addFile('Instalacao Padrao (Standard Install)'   , 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			replace = {'audio.':'[COLOR red][AUDIO] [/COLOR]', 'metadata.':'[COLOR cyan][METADATA] [/COLOR]', 'module.':'[COLOR red][MODULE] [/COLOR]', 'plugin.':'[COLOR blue][PLUGIN] [/COLOR]', 'program.':'[COLOR red][PROGRAM] [/COLOR]', 'repository.':'[COLOR gold][REPO] [/COLOR]', 'script.':'[COLOR green][SCRIPT] [/COLOR]', 'service.':'[COLOR green][SERVICE] [/COLOR]', 'skin.':'[COLOR dodgerblue][SKIN] [/COLOR]', 'video.':'[COLOR red][VIDEO] [/COLOR]', 'weather.':'[COLOR red][WEATHER] [/COLOR]'}
			for rep in replace:
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		...
		if warning == True:
			...
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Gostaria de BAIXAR e [COLOR green]INSTALAR[/COLOR] ?' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]INSTALAR[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			...
			if int(float(percent)) > 0:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s][B][COLOR yellow]ATENCAO!!! [/COLOR][/B] [COLOR %s]Para finalizar[/COLOR] INSTALACAO [COLOR %s]pressione [COLOR green]Encerrar KODI[/COLOR], caso o [COLOR white]KODI[/COLOR] nao feche automaticamente, jamais use a opcao [B]SAIR[/B], simplesmente [COLOR white]DESLIGUE[/COLOR] seu dispositivo da [COLOR yellow]ENERGIA ELETRICA[/COLOR] por 20s e reinicie o [COLOR white]KODI[/COLOR][/COLOR].[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recarregar Perfil[/COLOR][/B]", nolabel="[B][COLOR green]Encerrar Kodi[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
...
def freshStart(install=None, over=False):
	...
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce deseja LIMPAR e RESTAURAR o seu" % COLOR2, "KODI para as configuracoes padroes antes de ATUALIZAR com a NOVA Versao: [COLOR %s]%s[/COLOR]?" % (COLOR1, install), nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Continue[/COLOR][/B]')
