...
AddonID = 'plugin.video.miraculusof'
...
addon_data_dir = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.miraculusof/data/" ).decode("utf-8"), AddonID)
...
def gdrive(name,url,iconimage):
        tam = len(url)
        utam = tam - 28
        gdrive = "plugin://plugin.video.gdrive?mode=streamURL&amp;url=https://docs.google.com/file/d/"+url[utam:tam]+""
        PlayUrl(name,gdrive, iconimage)

def googledocs(name,url,iconimage):
        tam = len(url)
        utam = tam - 28
        gdrive = "plugin://plugin.video.gdrive?mode=streamURL&amp;url="+url+""
        PlayUrl(name,gdrive, iconimage)


def playlist_loader_mode(name, url, iconimage):
	prefix = url
        PlayUrl(name,prefix, iconimage)

def playlist_loader_sony(name, url, iconimage):
        url_sony =''+url+' |User-Agent=Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0'
        PlayUrl(name,url_sony, iconimage)
...
def player(name, url, iconimage):
	...
	if url[8:9] == "vi":
        ...
        if url[30] == "v":
                #https://www.youtube.com/watch?v=d2yk7Ih0LtM
        ...
        if tam == 61:
                gdrive(name,url,iconimage)

        if tam == 58 or url[8]== "d":
                googledocs(name,url,iconimage)
...
def play_loader(name,baseurl, iconimage):

        keyplay = xbmc.Keyboard('', 'Escolha Playlist Loader Modo 1 GX ou 2 Normal 3 Sony agent: ')
        keyplay.doModal()
        play = keyplay.getText()

	if play =="1":
                player(name, url, iconimage)

        if play =="2":
                playlist_loader_mode(name, url, iconimage)
				
	if play =="3":
                playlist_loader_sony(name, url, iconimage)
