...
addon = xbmcaddon.Addon('plugin.video.fdj.hd')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.fdj.hd-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(FDJ.hd, code error - "+str(e.code)+",10000,"+icon+")")
...
def getSearchData(url,icon, data=None):
    ...
    if keyboard.isConfirmed():
        ...
        xbmcgui.Dialog().ok('[COLOR white][B]FDJ[COLOR lime].[COLOR white]HD[/B][/COLOR]', '[COLOR white][B]Blank Searches are not allowed.[/B][/COLOR]')
        quit()
    fanart=''
    dontLink=False
    os.path.join(home, 'resources', 'fanart.gif')
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':           
                ...
                contextMenu.append(('[B][COLOR blue]Remove FDJ Favourite[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('[B][COLOR lime]Add to FDJ Favourites[/COLOR][/B]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('[B][COLOR blue]Remove FDJ Favourite[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('[B][COLOR lime]Add to FDJ Favourites[/COLOR][/B]','XBMC.RunPlugin(%s)' %fav_params))                                                                                
            liz.addContextMenuItems(contextMenu)
        try:
