...
addon = xbmcaddon.Addon(id='plugin.video.fenix.evolution')
...
def SKindex():
    addon_log("SKindex")
    getData(ksd,'')
    msg2 = "https://pastebin.com/raw/w60bATWT"				
    msg = msg2 
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('FENIX Seelecione uma fonte', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#                    print sepate
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                ...
                iloop+=1
            dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR firebrick][B]FENIX[/B][/COLOR] [COLOR white]EVOLUTION[/COLOR] - Selecione uma Opção: Ver Trailer ou assistir Filme', names)
            if index >= 0:
                playname=names[index]
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false":
                ...
                name = name + '[COLOR firebrick] (' + str(len(playlist)) + ' items )[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
